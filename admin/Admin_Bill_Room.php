...
if(isset($_GET["Name"])){
 ...
 if(isset($_GET["roomBID"])){
  $roomBID = $_GET['roomBID'];
 }
...
$sqlFn = ("SELECT fn_get_price_by_days('$rtID','$checkin','$checkout')");
...
<form method ="POST" action="">
 ...
 <label for="subject">Room Rent</label>
 <input  style="width:90%"  class="restbill" name="restbill" value ="<?php echo $troomRent; ?>" type="number" required></input>
 </div>
               ...
               if($custloyalty > $fetchPoint[0]){
                ...
                if($restbill){
                  ...
                  $finalBill = $finalBillwtax - $discount;
                   ...
                   $sql_invoice = "CALL sp_invoice_room('$cid','Room','$custloyalty','$finalBill','$roomBID')";
                   $user->db->next_result(); 
                   if(mysqli_query($user->db,$sql_invoice)){
                   echo " <br>
                   <a href=Print_Room-Bill.php?cid=".$cid."&roomamount=".$restbill."&loyalty=".$custloyalty."&tax=".$tax."&discount=".$discount."&finalBill=".$finalBill."&rtID=".$rtID."><button class='btn btn-block btn-secondary'> Print Bill</button> <a>";
                  }else{
