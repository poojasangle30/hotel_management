...
@page { margin: 0; }
  ...
  if(isset($_GET["cid"])){
   ...
   if(isset($_GET["roomamount"])){
     $roomamount = $_GET['roomamount'];
    }
    ...
    $sqlFn = ("CALL sp_get_deals_Details_in_room_bill('$cid','$rtID')");
	$resFn =  mysqli_query($user->db,$sqlFn);
	$user->db->next_result(); 
    if($re=mysqli_fetch_array($resFn))
    {
		$id = $re['deal_id'];
		$sqln = ("CALL sp_update_deals_isbooked_after_payment('$id')");
		$resn =  mysqli_query($user->db,$sqln);
		$user->db->next_result(); 
        $dealsDetails = "Breakfast + Dinner 1st night";
    }else{
        $dealsDetails = "No Deals";
    }
	...
	?>
		...
		<article>
			...
			<table >
				...
				<tbody>
							<tr>
								<td><span >Room Bill</span></td>
								<td><span >&nbsp;€ <?php echo number_format($roomamount, 2) ?> </span></td>
							</tr>
                    ...
                    <hr>
                    <tr>
						<td><span> Deals Applied </span></td>
						<td><span >&nbsp; <?php echo $dealsDetails; ?></span></td>
