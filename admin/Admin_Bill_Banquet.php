...
if(isset($_GET["Name"])){
 ...
 if(isset($_GET["banquet_booking_id"])){
  $banquet_booking_id = $_GET['banquet_booking_id'];
 }
 if(isset($_GET["banquet_capacity_price"])){
  $banquet_capacity_price = $_GET['banquet_capacity_price'];
 }
$custloyalty =0;
$restbill=0;
...
<form method ="POST" action="">
 ...
 <label for="subject">Banquet Bill</label>
 <input  style="width:90%"  value ="<?php echo $banquet_capacity_price; ?>" class="restbill" name="restbill" type="number"></input>
 </div>
               ...
               if($custloyalty > $fetchPoint[0]){
                ...
                if($restbill){
                  ...
                  if($custloyalty == 100){
                    ...
                    $restN = $multipleNumber - 1 ; // rest 0.05%;
                    $discount = 0.15 + ($restN * 0.05);
                    $t= $discount * $finalBill;
                    $finalBill = $finalBill - $t;
                     $finalBill;
                   
                  }
                  $sql_invoice = "CALL sp_invoice_banquet('$cid','Banquet','$custloyalty','$finalBill','$banquet_booking_id')";
                  $user->db->next_result(); 
                  if(mysqli_query($user->db,$sql_invoice)){
                    echo " <br>
                    <a href=Print_Rest-Bill.php?cid=".$cid."&restamount=".$restbill."&loyalty=".$custloyalty."&tax=".$tax."&discount=".$discount."&finalBill=".$finalBill."&billType=banquetBill"."><button class='btn btn-block btn-secondary'> Print Bill</button> <a>";
                  }else{
