...
<form method ="POST" action="">
 ...
 <label for="subject">Spa Bill</label>
 <input  style="width:90%"  class="spabill" name="spabill" type="number"></input>
 </div>
               ...
               if($custloyalty > $fetchPoint[0]){
                ...
                if($spabill){
                  ...
                  if($custloyalty == 100){
                    ...
                    $spaN = $multipleNumber - 1 ; // rest 0.05%;
                    $discount = 0.15 + ($spaN * 0.05);
                    $t= $discount * $finalBill;
                    $finalBill = $finalBill - $t;
                     $finalBill;
                   
                  }
                  $sql_invoice = "CALL sp_invoice_spa('$cid','tspa','$custloyalty','$finalBill')";
                  $user->db->next_result(); 
                  if(mysqli_query($user->db,$sql_invoice)){
                    echo " <br>
                    <a href=Print_Spa-Bill.php?cid=".$cid."&spaamount=".$spabill."&loyalty=".$custloyalty."&tax=".$tax."&discount=".$discount."&finalBill=".$finalBill."><button class='btn btn-block btn-secondary'> Print Bill</button> <a>";
                  }else{
